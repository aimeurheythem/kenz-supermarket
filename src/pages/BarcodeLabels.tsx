...
import { Search, Printer, Trash2, Plus, Barcode, ShoppingBag, Settings2 } from 'lucide-react';
        ...
        setQueue((prev) => {
            ...
            if (existing) {
                return prev.map((item) =>
                    item.originalProduct.id === product.id ? { ...item, quantity: item.quantity + 1 } : item,
                );
            }
        ...
        <div className="relative flex flex-col h-full gap-8 p-6 lg:p-8 animate-fadeIn mt-4 min-h-[85vh]">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 print:hidden">
                ...
                >
                    ...
                    <span>
                        {t('barcode_labels.print_labels')} ({totalLabels})
                    </span>
                </Button>
            </div>
                                ...
                                >
                                    {filteredProducts.map((product) => (
                                        ...
                                        >
                                            <div className="flex flex-col min-w-0">
                                                <span className="font-bold text-sm text-zinc-900 group-hover:text-blue-900 uppercase tracking-tight truncate">
                                                    {product.name}
                                                </span>
                                                <span className="text-[10px] text-zinc-400 font-mono font-bold">
                                                    {product.barcode || t('barcode_labels.no_barcode')}
                                                </span>
                                            </div>
                                    ...
                                    {filteredProducts.length === 0 && (
                                        <div className="text-center py-8 text-zinc-400 text-xs font-bold uppercase tracking-wider">
                                            {t('barcode_labels.no_products_found')}
                                        </div>
                                    )}
                                </motion.div>
                            )}
                                ...
                                {queue.map((item) => (
                                    ...
                                    >
                                        <div className="flex-1 min-w-0">
                                            <p className="font-bold text-xs text-zinc-900 uppercase tracking-tight truncate">
                            ...
                            <div className="space-y-3">
                                ...
                                >
                                    ...
                                    <SelectContent className="bg-white border-2 border-zinc-200 rounded-[1.5rem]">
                                        ...
                                        <SelectItem value="thermal_40x25">
                                            {t('barcode_labels.formats.thermal_40x25')}
                                        </SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            ...
                            <div className="space-y-4 pt-8">
                                <div className="flex items-center space-x-3">
                                    <Checkbox
                                        id="showPrice"
                                        checked={showPrice}
                                        onCheckedChange={(c: boolean | 'indeterminate') => setShowPrice(c === true)}
                                ...
                                <div className="flex items-center space-x-3">
                                    <Checkbox
                                        id="showName"
                                        checked={showName}
                                        onCheckedChange={(c: boolean | 'indeterminate') => setShowName(c === true)}
                                        className="h-6 w-6 rounded-lg border-2 border-zinc-300 data-[state=checked]:bg-yellow-400 data-[state=checked]:border-yellow-400"
                                    />
                                    <label
                                        htmlFor="showName"
                    ...
                    <div className="flex-1 bg-zinc-100 rounded-[3rem] border-3 border-dashed border-zinc-200 p-8 min-h-[400px] overflow-auto flex justify-center items-start shadow-inner">
                        <div
                            className={cn(
                                ...
                                labelFormat === '5160'
                                    ? 'w-[215.9mm] min-h-[279.4mm] p-[10mm]'
                                    : labelFormat === 'thermal_50x30'
                                      ? 'w-[50mm] min-h-[30mm] p-1'
                        ...
                        >
                            <div
                                className={cn(
                                    'grid',
                                    labelFormat === '5160'
                                        ? 'grid-cols-3 gap-x-[4mm] gap-y-[0mm]'
                                        : 'grid-cols-1 gap-1',
                                )}
                            >
                                {labelArray.map((product, idx) => (
                                    ...
                                    >
                                        {showName && (
                                            <p
                                                className={cn(
                                                    'font-bold leading-tight truncate w-full shrink-0',
                                                    labelFormat === '5160'
                                                        ? 'text-[9px] mb-[1px]'
                                                        : 'text-[10px] mb-0.5',
                                                )}
                        ...
                        <div
                            ...
                            className={cn(
                                ...
                                labelFormat === '5160'
                                    ? 'h-[25.4mm] px-2'
                                    : labelFormat === 'thermal_50x30'
                                      ? 'h-[30mm] px-1 page-break-after-always'
                        ...
                        >
                            {showName && (
                                <p
                                    className={cn(
                                        'font-bold leading-tight truncate w-full shrink-0',
                                        labelFormat === '5160' ? 'text-[9px] mb-[1px]' : 'text-[10px] mb-0.5',
                                    )}
                                >
                                    {product.name}
                                </p>
                            ...
                            <BarcodeCanvas
                                ...
                                className={cn(
                                    'object-contain',
                                    labelFormat === '5160' ? 'max-w-full max-h-[14mm]' : 'w-full max-w-[95%]',
                                )}
                            />
                            ...
                            {showPrice && (
                                <p
                                    className={cn(
                                        'font-black shrink-0',
                                        labelFormat === '5160' ? 'text-[10px] mt-[1px]' : 'text-[11px] mt-0.5',
                                    )}
                                >
                                    {formatCurrency(product.selling_price)}
                                </p>
