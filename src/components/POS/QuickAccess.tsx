    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <span className="text-[12px] text-black uppercase tracking-[0.3em] font-bold">{t('pos.quick_access.title')}</span>
                {user?.role === 'admin' && (
                    <button
            ...
            {items.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 pb-4">
                    {items.filter(Boolean).map((item) => (
                        (Array.isArray(item.options) ? item.options : []).map((opt, optIdx) => {
                            ...
                            const prod = products.find(p => p.id === item.product_id);
                            const inCart = cart.find(c => c.product.id === item.product_id && Math.abs(c.product.selling_price - (opt.price / opt.qty)) < 0.01);
                                ...
                                >
                                    ...
                                    <div className="relative z-10 flex items-center justify-between">
                                        <span className={cn(
                                            ...
                                            {opt.qty > 1 ? t('pos.quick_access.units', { count: opt.qty }) : t('pos.quick_access.single_unit')}
                                        </span>
                                    ...
                                    <div className="relative z-10 flex items-center gap-3">
                                        <div className={cn(
                                            ...
                                            <AnimatePresence mode="wait">
                                                {isAdded || inCart ? (
                                                    ...
                                                    >
                                                        <Check className={inCart ? "text-black" : "text-white group-hover:text-black"} size={20} strokeWidth={3} />
                                                    </motion.div>
                                                ) : (
                                                    <motion.div
                                                        key="bag"
                                                        ...
                                                        exit={{ scale: 0 }}
                                                    >
                                                        <ShoppingBag className="text-white group-hover:text-black" size={20} />
                                                    </motion.div>
                                                )}
                                            </AnimatePresence>
                                        </div>
                                        <div className="space-y-0.5">
                                            <h3 className={cn(
                                                ...
                                                inCart ? "text-black/40" : "text-white/40 group-hover:text-black/40"
                                            )}>
                ...
                <div className="col-span-full py-12 border-2 border-dashed border-zinc-100 rounded-[3rem] flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <div>
                        <h3 className="font-black text-zinc-400 uppercase tracking-tight">{t('pos.quick_access.no_items')}</h3>
                        <p className="text-sm text-zinc-300 font-bold uppercase tracking-wider">{t('pos.quick_access.no_items_hint')}</p>
                    </div>
                    {user?.role === 'admin' && (
                        <Button onClick={() => setIsManagerOpen(true)} className="bg-black text-white px-8 rounded-2xl h-12">
